module.exports = function (RED) {
        ...
        node.on('input', function (msg) {
            ...
            if (node.cmd == "filter")
            {
                ...
                if (node.enableFil == 1){
                    ...
                    if (node.inputFil)
                    {
                        listFiltr.push("input");
                    }
                    if (node.relayFil || node.digoutFil)
                    {
                        listFiltr.push("relay");
                    }
            ...
            if (node.cmd == "set") {
                if(node.alias !== "")
                    ...
                    if (node.devices == "relay" || node.devices == "led") {
    ...
    RED.nodes.registerType("unipi-output", UnipiOutputNode);
}